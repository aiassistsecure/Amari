...
//
//   node amari.mjs scan              # scan all watchlist items via the MCP
//   node amari.mjs scan --only=mcp_seekers --limit=15 --verbose
//   node amari.mjs draft             # write warm openers for undrafted leads
//   node amari.mjs draft --limit=10
//   node amari.mjs list              # show recent leads
//   node amari.mjs list --status=drafted
//   node amari.mjs report            # export the queue to a PDF
//   node amari.mjs report --out=./reports/today.pdf
//   node amari.mjs stats             # quick counters
//   node amari.mjs ignore <lead_id>  # mark a lead ignored
//
// Env:
...
function printUsage() {
  process.stdout.write([
    `${C.bold}usage:${C.reset} node amari.mjs <command> [flags]`,
    ``,
...
try {
  if (cmd === "scan") {
    ...
    conn.close();
    if (!rows.length) { console.log(`${C.dim}(no leads yet — run: node amari.mjs scan)${C.reset}`); }
    for (const r of rows) {
    ...
    if (!id) { console.error("usage: node amari.mjs ignore <lead_id>"); process.exit(2); }
    const conn = db.open();
